...
- **Tests:** 551 pytest tests (550 passing, 1 skipped — a Windows-admin-only symlink test). Includes 21 security-bypass tests and 11 dedicated spoliation tests.
...
5. Test suite summary screenshot (`pytest tests/ -v` tail showing 550 passed / 1 skipped — 551 total).
...
## Test Suite Summary
551 tests total — 550 passing, 1 skipped (Windows-admin-only symlink test; passes on Linux/SIFT).
...
```bash
# From the repo root
pytest tests/ -v                             # expect 550 passed, 1 skipped (551 total)
